 ...
 * Name: countNodes
 * Return Type: int
 * Parameters: TreeNode* nodePtr
 * Description: counts all the nodes in an avl tree and returns as integer
 */
template <class T> int AVLTree<T>::countNodes(TreeNode* nodePtr) {
  if (!nodePtr)
    return 0;
 ...
 * Name: getIPL
 * Return Type: double
 * Parameters: TreeNode* nodePtr, int height
 * Description: Recursively gets the IPL
 */
template <class T> double AVLTree<T>::getIPL(TreeNode* nodePtr, int height) {
  if (!nodePtr)
    return 0;
 ...
 */
template <class T> double AVLTree<T>::getIPL() {
