...
protected:
	...
	int countNodes(RBTreeNode<T>* nodePtr);
	double getIPL(RBTreeNode<T>* nodePtr, int height);
 ...
 * Name: countNodes
 * Return Type: int
 * Parameters: RBTreeNode* nodePtr
 * Description: counts all the nodes in an avl tree and returns as integer
 */
template <class T> int RBTree<T>::countNodes(RBTreeNode<T>* nodePtr) {
  if (nodePtr == NIL)
    return 0;
 ...
 * Name: getIPL
 * Return Type: double
 * Parameters: RBTreeNode* nodePtr, int height
 * Description: Recursively gets the IPL
 */
template <class T> double RBTree<T>::getIPL(RBTreeNode<T>* nodePtr, int height) {
  if (nodePtr == NIL)
    return 0;

  return height + getIPL(nodePtr->left, height + 1) + getIPL(nodePtr->right, height + 1);
}
 ...
 * Return Type: double
 * Parameters: None
 * Description: divides recursive getIPL by countNodes to get the IPL of the tree
 */
template <class T> double RBTree<T>::getIPL() {
  double pathLengths = getIPL(root, 0);
  int n = countNodes(root);
  return pathLengths / static_cast<double>(n);
}
