...
public:
    ...
    RBTreeNode<K, V>* findNode(const K & item, const V & value);
...
void RBTree<K, V>::remove(K key, V value) {
    RBTreeNode<K, V> *z = findNode(key, value);
    if (z == NIL)
        return;
 ...
 */
template<class K, class V>
RBTreeNode<K, V>* RBTree<K, V>::findNode(const K & item, const V & value) {
    RBTreeNode<K, V> *nodePtr = root;

    while (nodePtr != NIL) {
        if (nodePtr->key == item && nodePtr->value == value)
            return nodePtr;
        else if (item < nodePtr->key)
            nodePtr = nodePtr->left;
        else
