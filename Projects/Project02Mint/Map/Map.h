...
public:
    ...
    friend ostream & operator<<(ostream & os, Map<K, V> & map);
};
...
template <class K, class V>
Map<K, V>::Map(const Map& obj) : RBTree<K, V>(obj) {
...
*/
template <class K, class V>
int Map<K, V>::size(RBTreeNode<K,V>* nodePtr) {
    if (nodePtr == this->NIL)
        return 0;
 ...
 * Parameters: Map<K, V> & rhs
 * Return Type: bool
 */
template <class K, class V>
bool Map<K, V>::operator==(Map<K, V> & rhs) {
 ...
 */
template <class K, class V>
void Map<K, V>::toVector(RBTreeNode<K, V>* nodePtr, vector<pair<K, V>> & vect) {
    if (nodePtr != this->NIL) {
 ...
 */
template <class K, class V>
void Map<K, V>::toVector(vector<pair<K, V>> & vect) {
    if (!vect.empty())
        vect.clear();
 ...
 * Parameters: ostream & os, Map<K, V> & map
 * Return Type: ostrea &
 */
template <class K, class V>
ostream & operator<<(ostream & os, Map<K, V> & map) {
